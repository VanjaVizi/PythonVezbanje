...
def collect_athletes_data(athletes_names):
    ...
    for name in athletes_names:
        ...
        if country:
            athletes_data.append((name, country))
        else:
            print(f"Could not retrieve birthplace for: {name}")

    to_csv(Path.cwd() / ORIGINS_CSV_FILE, ["athlete_name", "country"], athletes_data)
...
def most_represented_countries(athletes_list):
    ...
    print("Country representation among top athletes:")
