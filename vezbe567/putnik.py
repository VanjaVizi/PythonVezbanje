...
class Putnik:
    ...
    @property
    def pasos(self):
        if not hasattr(self, '_Putnik__pasos'):  #Look Before You Leap
            self.__pasos = None
        return self.__pasos

        try:  # easier to ask for forgiveness than permission - EAFP
            return self.__pasos
    ...
    def pasos(self, value):
        ...
        if isinstance(value, int) and len(str(value)) == 6:
            ...
            return
        print(f"Iz pasos.setter: Greska pri postavljanju vrednosti atributu 'pasos': "
    ...
    def __eq__(self, other):
        ...
        print("Iz __eq__: Nije moguce uporediti dva data objekta klase Putnik jer bar jedan od njih "
    ...
    def azuriraj_COVID_bezbedan(self, tip_uverenja, datum_uverenja):
        if (not isinstance(tip_uverenja, str)) or (tip_uverenja.lower() not in ['vakcinacija', 'negativan_test']):
            print("Iz azuriraj_COVID_bezbedan: pogresna vrednost za ulazni argument 'tip_uverenja'\n")
            return
        if not isinstance(datum_uverenja, (str, datetime)):
            print("Iz azuriraj_COVID_bezbedan: pogresna vrednost za ulazni argument 'datum_uverenja'\n")
            return
        if isinstance(datum_uverenja, str):
            datum_uverenja = datetime.strptime(datum_uverenja, '%d/%m/%Y')

        dt_delta = datetime.now() - datum_uverenja  #dt_delta.days     .seconds


        if (tip_uverenja.lower()=="vakcinacija" and dt_delta.days < 365 ):
            self.COVID_bezbedan=True
        if (tip_uverenja.lower()=="negativan_test" and dt_delta.days < 3 ):
            self.COVID_bezbedan=True
    ...
    @classmethod
    def covid_bezbedan_Francuz(cls, ime, pasos):
